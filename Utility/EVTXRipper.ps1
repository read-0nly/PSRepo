...
function introFetch(){
    introwatermark(-1) 
    $workDir  =  (read-host "Please enter the path to the directory containing mdmdiagnosticstool output ('.' is the alias of the current directory)")
}
...
function introWatermark($i){
    ...
    $wmColor =@("Red","Yellow","Yellow","Yellow", "Green")
    cls
...
function runEvtxRipper(){
    ...
    $AllUniques | sort-object timecreated | select-object -property $selection | out-gridview
    introWatermark(3)
    $AllEventsPath = $null
...
while(($workDir -eq $null)){
    introfetch    
    if(-not ($workDir -eq $null)){
        while (-not (test-path $workDir) -or (-not ((dir $workDir+'\*.evtx').count -gt 0))){
            introFetch
        }
    }
}
